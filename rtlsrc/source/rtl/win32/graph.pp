{
    $Id: graph.pp,v 1.1.2.5 2002/09/09 12:29:41 pierre Exp $
...
const
   MessageThreadHandleValid : boolean = false;
...
{$ifdef debug}
  { special stderr for graph thread }
  graphstderr : Text;
{$endif}
                 ...
                 if bitmapfonthorizoncache[byte(textstring[i+1])]=0 then
                   begin
                      ...
                      if charbitmap=0 then
                        writeln('Could not create bitmap!');
...
procedure SetRGBPaletteWin32GUI(colorNum,redValue,greenvalue,
  ...
  begin
     if directcolor or (colornum<0) or (colornum>=maxcolor) or not graphrunning then
       begin
         _graphresult:=grerror;
         exit;
       end;
...
{$endif DEBUG_WM_PAINT}
    ...
    wm_ncDestroy:
      begin
{$ifdef debug}
        writeln(graphstderr,'wm_ncdestroy recieved');
{$endif}
        WindowProc := DefWindowProc(Window, Windows.UInt(AMessage), Windows.WParam(WParam), Windows.LParam(LParam));
      end;
    wm_close:
      begin
{$ifdef debug}
         writeln(graphstderr,'wm_close recieved');
{$endif}
    ...
    wm_Destroy:
      begin
{$ifdef debug}
         writeln(graphstderr,'wm_destroy recieved');
{$endif}
         EnterCriticalSection(graphdrawing);
         graphrunning:=false;
...
begin
  ...
  hWindow := CreateWindow('FPCGraphWindow', windowtitle,
              ws_OverlappedWindow or ws_visible or extrastyle, CW_USEDEFAULT, 0,
...
function MessageHandleThread(p : pointer) : DWord;StdCall;
  ...
  begin
     ...
     PeekMessage(@AMessage,0,WM_USER,WM_USER,PM_NOREMOVE);
     SetEvent(graphinit);
     if longint(mainwindow) = 0 then begin
       MessageBox(0, 'Window creation failed', nil, mb_Ok);
       ExitThread(1);
     end;
     Msg:=longint(GetMessage(@AMessage, 0, 0, 0));
     while Msg=longint(true) do
       begin
{$ifdef debug}
          writeln(graphstderr,'Message ',AMessage.message,' recieved');
{$endif}
...
{$ifdef debug}
     writeln(graphstderr,'Last message ',AMessage.message,' recieved');
{$endif}
     MessageHandleThread:=AMessage.wParam;
  end;
...
procedure InitWin32GUI16colors;
  ...
  begin
{$ifdef debug}
     writeln(stderr,'InitWin32GUI16colors called');
{$endif}
     if graphrunning then
       exit;
     ...
     graphinit:=CreateEvent(nil,false,false,nil);
     graphrunning:=false;
...
{$ifdef debug}
     writeln(stderr,'InitWin32GUI16colors succeeded');
{$endif}
        end
     else
       MessageThreadHandleValid:=true;

     WaitForSingleObject(graphinit,Infinite);
     CloseHandle(graphinit);
     graphinit:=0;
  end;
...
{$ifdef debug}
     writeln(stderr,'CloseGraph called');
{$endif}
     if graphrunning then
       begin
{$ifdef debug}
         writeln(stderr,'sending wm_close to MainWindow');
{$endif}
         res:=PostMessage(MainWindow,wm_close,0,0);
         if not res then
           begin
             error:=GetLastError;
{$ifdef debug}
             writeln(graphstderr,'sending wm_close to MainWindow ',error);
{$endif}
             Sleep(1);
             res:=PostMessage(MainWindow,wm_close,0,0);
           end;
{$ifdef debug}
         writeln(stderr,'Waiting for MessageThreadHandle to finish');
{$endif}
         WaitForSingleObject(MessageThreadHandle,Infinite);
       end;
     if MessageThreadHandleValid then
       begin
         MessageThreadHandleValid:=false;
         CloseHandle(MessageThreadHandle);
         DeleteCriticalSection(graphdrawing);
       end;
     isgraphmode := false;
...
function queryadapterinfo : pmodeinfo;
  ...
  procedure SetupWin32GUIDefault;
    ...
    begin
       ...
       mode.PutImage := {$ifdef fpc}@{$endif}PutImageWin32GUI;
...
{$ifdef debug}
  Assign(graphstderr,'');
  Rewrite(graphstderr);
  TextRec(graphstderr).Handle:=longint(GetStdHandle(STD_ERROR_HANDLE));
{$endif}
  InitializeGraph;
end.
{
  $Log: graph.pp,v $
  Revision 1.1.2.5  2002/09/09 12:29:41  pierre
   * fix the closing of GraphWindow in CloseGraph

  Revision 1.1.2.4  2002/04/11 21:30:18  pierre
   * use ThreadId as argument of PostThreadMessage

  Revision 1.1.2.3  2002/04/11 20:39:08  pierre
   * make new windows after the first one also visible

  Revision 1.1.2.2  2002/01/06 15:22:06  florian
    * SetRGBColor with cached pens fixed

  Revision 1.1.2.1  2001/06/06 14:27:14  jonas
