{
    $Id: palm.pp,v 1.1.2.3 2003/04/24 09:48:31 pierre Exp $
    This file is part of the Free Pascal run time library.
    Copyright (c) 1999-2000 by H.M. Swartjes
    Conversion to Free Pascal copyright (c) 1999-2001 by
...
{ ------------- Evt ------------- }
   ...
   procedure EvtGetEvent (var event: EventType;timeOut: LongInt);
...
{ ------------- Frm ------------- }
   ...
   function FrmAlert (alertID: Integer;Arg1,Arg2,Arg3 : pchar): Integer;
      syscall SysTrapFrmCustomAlert;

   procedure FrmCloseAllForms;
      syscall SysTrapFrmCloseAllForms;

   procedure FrmDeleteForm (frm: PFormtype);
      syscall SysTrapFrmDeleteForm;

   procedure FrmDoDialog (theForm: PFormtype);
      syscall SysTrapFrmDoDialog;

   procedure FrmDrawForm (theForm: PFormtype);
      syscall SysTrapFrmDrawForm;

   function FrmGetActiveForm: PFormtype;
      syscall SysTrapFrmGetActiveForm;

   function FrmGetActiveFormID: Integer;
      syscall SysTrapFrmGetActiveFormID;

   function FrmGetFocus (theForm: PFormtype): Integer;
      syscall sysTrapFrmGetFocus;

   function FrmGetFormPtr (id: Integer): PFormtype;
      syscall SysTrapFrmGetFormPtr;

//   function FrmGetObjectID (ObjIndex: Integer; frm: PFormtype): Integer;
//      syscall SysTrapFrmGetObjectID;

   function FrmGetObjectIndex (frm: PFormtype; ObjID: Integer): Integer;
      syscall SysTrapFrmGetObjectIndex;

   function FrmGetObjectPtr (frm: PFormtype; ObjIndex: Integer): Ptr;
      syscall SysTrapFrmGetObjectPtr;

   procedure FrmGotoForm (frmID: integer);
      syscall SysTrapFrmGotoForm;

   function FrmHandleEvent (frm: PFormtype; var event: eventType): Integer;
      syscall SysTrapFrmHandleEvent;

//   procedure FrmHideObject (objIndex: Integer; frm: PFormtype);
//      syscall SysTrapFrmHideObject;

   function FrmInitForm (frmID: integer): PFormtype;
   ...
   procedure FrmSetActiveForm (theForm: PFormtype);
      syscall SysTrapFrmSetActiveForm;

//   procedure FrmSetFocus (fieldIndex: Integer; frm: PFormtype);
//      syscall sysTrapFrmSetFocus;

//   procedure FrmShowObject (objIndex: Integer; frm: PFormtype);
...
{ ------------- Menu ------------- }
   ...
   function MenuHandleEvent (menuP: Ptr; var event: eventType; var error: Word) : Integer;
...
//      syscall SysTrapSysFormPointerArrayToStrings;
   ...
   function SysHandleEvent (var event: eventType): Integer;
...
{ ------------- Tbl ------------- }
   ...
   procedure TblDrawTable (table: PTableType);
      syscall SysTrapTblDrawTable;

//   function TblGetItemInt (column, row: Integer; table: Ptr): Integer;
//      syscall SysTrapTblGetItemInt;

   function TblGetNumberOfRows (table: PTableType): Integer;
      syscall SysTrapTblGetNumberOfRows;

   procedure TblSetColumnUsable (table: PTableType; row: Integer;usable: boolean);
...
//      syscall SysTrapTblSetItemPtr;
   ...
   procedure TblSetItemStyle (table: PTableType; row, column, style : Integer);
...
//      syscall SysTrapTblSetRowSelectable;
   ...
   procedure TblSetRowUsable (table: PTableType; row: Integer;usable: boolean);
...
//      syscall SysTrapDayOfWeek;
   ...
   procedure TimSecondsToDateTime (theSecs: Longint; var dt: DateTimeType);
      syscall SysTrapTimSecondsToDateTime;

   { ------------- Win ------------- }
   procedure WinDrawChars (theString: Ptr; len, x, y: Integer);
      syscall SysTrapWinDrawChars;

   procedure WinDrawGrayRectangleFrame (frameType: Integer; var theRect: Rectangletype);
      syscall SysTrapWinDrawGrayRectangleFrame;

   procedure WinDrawRectangle (var theRect: Rectangletype; cornerDiam: Integer);
      syscall SysTrapWinDrawRectangle;

   procedure WinEraseRectangle (var theRect: Rectangletype; corenerDiam: Integer);
      syscall SysTrapWinEraseRectangle;

   procedure WinFillRectangle (var theRect: Rectangletype; cornerDiam: Integer);
...
{
  $Log: palm.pp,v $
  Revision 1.1.2.3  2003/04/24 09:48:31  pierre
   + Add custom version of FrmAlert

  Revision 1.1.2.2  2003/01/10 15:43:08  pierre
   * fix typo
