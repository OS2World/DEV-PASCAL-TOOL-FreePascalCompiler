{
    $Id: profile.pp,v 1.1.2.2 2002/12/07 00:28:27 pierre Exp $
...
{ this must be below the first statement in interface,
  so that the default units are already loaded PM }
{$ifdef FPC_PROFILE}
{$error }
...
begin
   ...
   if (endtext=0) or (mcount_skip<>0) then
     asm
        popal
        leave
        ret
...
{
  $Log: profile.pp,v $
  Revision 1.1.2.2  2002/12/07 00:28:27  pierre
   * avoid recursive calls

  Revision 1.1.2.1  2002/09/09 09:20:03  pierre
   * profiling fixes
