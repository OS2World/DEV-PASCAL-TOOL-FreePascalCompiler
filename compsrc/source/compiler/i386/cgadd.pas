 {
    $Id: cgadd.pas,v 1.1.2.11 2003/01/15 17:59:51 florian Exp $
       ...
       equaln,xorn : begin
           ...
           begin
              ...
              if (p^.left^.treetype=ordconstn) or
                 (p^.left^.treetype=realconstn) then
               swaptree(p);
              secondpass(p^.left);
...
{$endif test_dest_loc}
              ...
              if p^.location.loc=LOC_FPU then
                pushedfpu:=maybe_pushfpu(p^.right^.registersfpu,p)
              else
                pushedfpu:=false;
...
{$endif noAllocEdi}
                         ...
                         { the location is released in emitloadord2reg already (FK)
                        ...
                        if (p^.left^.location.loc=LOC_CREGISTER) then
                          ...
                          begin
                             ...
                             if is_in_dest then
                               ...
                               begin
                                  { first release one, then demand new register }
                                  case opsize of
                                     S_L : hregister:=getregister32;
               ...
               if (p^.left^.resulttype^.deftype=floatdef) and
                 ...
                 begin
                    ...
                    if (p^.left^.treetype=realconstn) and
                      ...
                      swaptree(p); obsolete, already swaped above PM }
                    cmpop:=false;
                    case p^.treetype of
                       addn : op:=A_FADDP;
                       muln : op:=A_FMULP;
...
{
  $Log: cgadd.pas,v $
  Revision 1.1.2.11  2003/01/15 17:59:51  florian
    * tb0427 fixed

  Revision 1.1.2.10  2002/12/12 13:14:44  pierre
   * avoid to generate shl #0,reg useless instructions

  Revision 1.1.2.9  2002/07/29 14:24:23  jonas
    * fixed web bug 2031

  Revision 1.1.2.8  2002/04/05 15:01:12  jonas
    * fixed web bug 1915

  Revision 1.1.2.7  2001/09/10 23:05:01  pierre
   * avoid fpu stack overflow

  Revision 1.1.2.6  2001/08/29 14:55:25  jonas
  ...
  Revision 1.1.2.5  2001/06/25 13:38:28  jonas
    * fixed set bug discovered by Carl

  Revision 1.1.2.4  2001/06/16 03:43:47  carl
  - useless define removed

  Revision 1.1.2.3  2001/05/06 17:12:13  jonas
    * fixed an IE10 and another bug with [var1..var2] construct

  Revision 1.1.2.2  2001/02/27 02:15:50  carl
  * rename maybe_loadesi to maybe_loadself

  Revision 1.1.2.1  2001/02/25 03:44:39  carl
  + trying again
