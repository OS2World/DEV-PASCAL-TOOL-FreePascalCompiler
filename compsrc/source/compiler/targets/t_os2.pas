{
    $Id: t_os2.pas,v 1.1.2.5 2003/03/17 13:35:39 peter Exp $
...
{$endif Delphi}
     ...
     globals,verbose,files,script,ppu;
...
begin
    ...
    seq_no:=1;
    current_module^.linkotherstaticlibs.insert(libname,link_allways);
...
{ Create some replacements }
  if (cs_link_strip in aktglobalswitches) then
   ...
   StripStr := '';
  if (usewindowapi) or (AppType = at_gui) then
   AppTypeStr := '-p'
  else if AppType = at_fs then
   AppTypeStr := '-f'
  else AppTypeStr := '-w';
