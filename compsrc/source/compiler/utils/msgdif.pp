{
    $Id: msgdif.pp,v 1.1.2.5 2003/01/05 15:53:46 florian Exp $
...
function MsgToSet(const Msg, FileName: string; var R: TArgSet): Boolean;
  ...
  begin
    ...
    for i:=1 to Length(Msg) do
      ...
      begin
        j:=i+1;
        while (Msg[j] in ['0'..'9']) and (j<length(msg)) do Inc(j);
        if j > i+1 then
        begin
...
procedure stripblocksep(var s : string);
       ...
       begin
         if not assigned(orgnext^.equivalent) then
           ...
           begin
             ...
             begin
               Write(t,stripblocksep(orgnext^.equivalent^.comment));
               ...
               begin
                 ...
                 Write(t,stripblocksep(orgnext^.comment));
                 inc(i,orgnext^.cnb);
               end;
           end;
...
{
  $Log: msgdif.pp,v $
  Revision 1.1.2.5  2003/01/05 15:53:46  florian
    + added proper support of type = type <type>; for simple types

  Revision 1.1.2.4  2002/09/04 14:35:09  pierre
   * add a check for different verbosity

  Revision 1.1.2.3  2001/03/10 12:59:14  peter
    * test arguments patch from Sergey applied.

  Revision 1.1.2.2  2001/03/05 21:43:57  peter
    * small diffs from Sergey applied

  Revision 1.1.2.1  2001/02/09 23:04:40  peter
