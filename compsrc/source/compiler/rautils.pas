{
    $Id: rautils.pas,v 1.1.2.8 2002/11/12 11:38:56 pierre Exp $
...
procedure AsmSearchSym(const s:string);
        ...
        case pvarsym(sym)^.owner^.symtabletype of
          ...
          localsymtable :
            begin
              ...
              if (pvarsym(sym)^.varspez in [vs_var]) or
                 ((pvarsym(sym)^.varspez=vs_const) and
                  push_addr_param(pvarsym(sym)^.vartype.def)) then
                SetSize(target_os.size_of_pointer,false);
            end;
...
begin
  ...
  if i>0 then
   begin
     getsym(Copy(s,1,i-1),false);
     if assigned(srsym) then
      begin
        if (srsym^.typ=unitsym) and
           (srsym^.owner^.unitid=0) then
         getsymonlyin(punitsym(srsym)^.unitsymtable,Copy(s,i+1,255))
        else
         srsym:=nil;
      end;
   end
...
{
  $Log: rautils.pas,v $
  Revision 1.1.2.8  2002/11/12 11:38:56  pierre
  ...
  Revision 1.1.2.7  2001/08/09 11:38:02  pierre
   * fix subbtle m68k specific bug due to the fact that symofs was a the same location as ref.segment

  Revision 1.1.2.6  2001/06/02 02:04:48  peter
    * support unit.variable (backported)

  Revision 1.1.2.5  2001/02/23 10:05:19  pierre
