{
    $Id: symtable.pas,v 1.1.2.26 2003/01/11 18:13:01 carl Exp $
...
{*** Misc ***}
    ...
    procedure resetscanners(todispose : pointer);
...
*****************************************************************************}
    ...
    procedure check_forward(sym : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
    ...
    procedure labeldefined(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
    ...
    procedure unitsymbolused(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
    ...
    procedure varsymbolused(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
    ...
    procedure TestPrivate(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
        if sp_private in psym(p)^.symoptions then
          varsymbolused(p);
      end;

    procedure objectprivatesymbolused(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
         {
...
{$ifdef GDB}
    procedure concatstab(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
        if psym(p)^.typ <> procsym then
          psym(p)^.concatstabto(asmoutput);
      end;

    procedure resetstab(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
        if psym(p)^.typ <> procsym then
          psym(p)^.isstabwritten:=false;
      end;

    procedure concattypestab(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
    ...
    procedure avoidtypestab(p : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
        if psym(p)^.typ = typesym then
         begin
           psym(p)^.isstabwritten:=true;
           if assigned(ptypesym(p)^.restype.def) then
             ptypesym(p)^.restype.def^.is_def_stab_written:=written;
         end;
      end;
...
{$endif DONOTCHAINOPERATORS}
    ...
    procedure write_refs(sym : pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
...
***********************************************}
   ...
   function tsymtable.getnewtypecount : longint;
    ...
    procedure order_overloads(p : Pnamedindexobject);{$ifdef tp}far;{$endif}
      begin
         if psym(p)^.typ=procsym then
...
{$endif GDB}
    ...
    function tsymtable.speedsearch(const s : stringid;speedvalue : longint) : psym;
      ...
      begin
        ...
        { skip private members when found in withsymtable }
        if assigned(hp) and
           (symtabletype=withsymtable) and
           (sp_private in hp^.symoptions) and
           (hp^.owner^.symtabletype=objectsymtable) and
           (hp^.owner^.defowner^.owner^.symtabletype=unitsymtable) then
         hp:=nil;
...
{$ifdef m68k}
             { only on enumdef's and ordinal definitions }
             if (sym^.vartype.def^.deftype in [orddef,enumdef]) then
               sym^.address:=datasize-l;
{$endif}
         ...
         if cs_gdb_dbx in aktglobalswitches then
           begin
             ...
             if t = globalsymtable then
               begin
                 dbx_index:=1;
                 debuglist^.concat(new(pai_asm_comment,init(strpnew('Global '+name^+' has index '+tostr(dbx_index)))));
...
{$ifdef GDB}
      ...
      function get_new_global_dbx_index : longint;
        begin
          if not assigned(current_module) or
             not assigned(current_module^.globalsymtable) then
            internalerror(20021120);
          inc(punitsymtable(current_module^.globalsymtable)^.dbx_index_last);
          get_new_global_dbx_index:=punitsymtable(current_module^.globalsymtable)^.dbx_index_last;
        end;
        ...
        begin
           if is_stab_written then
             begin
               if cs_gdb_dbx in aktglobalswitches then
                 begin
                   foreach({$ifndef TP}@{$endif}avoidtypestab);
                 end;
               exit;
             end;
...
****************************************************************************}
   ...
   procedure testfordefaultproperty(p : pnamedindexobject);{$ifdef tp}far;{$endif}
     begin
...
{
  $Log: symtable.pas,v $
  Revision 1.1.2.26  2003/01/11 18:13:01  carl
    * compilation bugfixes under BP

  Revision 1.1.2.25  2002/12/18 16:52:34  pierre
   * dbx specific changes

  Revision 1.1.2.24  2002/11/19 00:48:57  pierre
   * fix tbs/tb0419 cdecl open array problem

  Revision 1.1.2.23  2002/11/07 16:51:02  pierre
   * several memory leaks removed

  Revision 1.1.2.22  2002/10/22 10:12:40  pierre
   * try to improove with inlining (still only in debug mode)

  Revision 1.1.2.21  2002/10/18 10:47:13  pierre
   * small dbx change

  Revision 1.1.2.20  2002/10/08 13:54:22  pierre
   * bestrealdef must be the same type as extended for variable arrays

  Revision 1.1.2.19  2002/09/21 20:45:13  carl
    * cdecl calling conventions fixes (address in paramsym changed)
    * stackalign = 4 convention fixes (address in parasym changed)

  Revision 1.1.2.18  2002/03/01 21:02:17  carl
  + fixes for BP compilation

  Revision 1.1.2.17  2002/01/07 16:33:01  pierre
   * fix for bug 1740 affecting IDE

  Revision 1.1.2.16  2001/09/27 23:30:33  pierre
   * allow -gd option to compile complier again

  Revision 1.1.2.15  2001/08/17 16:16:45  florian
    + support for PalmOS added

  Revision 1.1.2.14  2001/07/29 22:12:02  peter
    * skip private symbols when found in withsymtable

  Revision 1.1.2.13  2001/07/25 15:57:49  pierre
   * change bestreal to s64floatdef

  Revision 1.1.2.12  2001/05/08 14:07:35  carl
  - revert back to 32-bit IEEE real for m68k

  Revision 1.1.2.11  2001/03/10 13:26:00  peter
    * small patch from Sergey to allow compilation with FPC in TP mode

  Revision 1.1.2.10  2001/01/08 21:38:30  peter
