{
    $Id: hcgdata.pas,v 1.1.2.5 2002/11/07 16:50:59 pierre Exp $
...
{$endif}
         ...
         len:=strlen(p^.p^.messageinf.str);
         datasegment^.concat(new(pai_const,init_8bit(len)));
         getmem(ca,len+1);
         move(p^.p^.messageinf.str^,ca^,len+1);
                                  ...
                                  if equal_paras(procdefcoll^.data^.para,hp^.para,cp_all) and
                                    ...
                                    begin { same parameters }
                                            ...
                                            if _c^.is_class then
                                              begin
                                                 ...
                                                 if _c=hp^._class then
                                                   MessagePos1(
                                                     hp^.fileinfo,parser_w_should_use_override,_c^.objname^+'.'+_name);
                                              end
                                            else
                                              if _c=hp^._class then
                                                begin
                                                   if (po_virtualmethod in procdefcoll^.data^.procoptions) then
                                                     MessagePos1(hp^.fileinfo,
                                                      parser_w_overloaded_are_not_both_virtual,_c^.objname^+'.'+_name)
                                                   else
                                                     MessagePos1(hp^.fileinfo,parser_w_overloaded_are_not_both_non_virtual,
                                                       _c^.objname^+'.'+_name);
                                                end;
                                       ...
                                       if (procdefcoll^.data^.proccalloptions<>hp^.proccalloptions) or
                                         ...
                                         MessagePos1(hp^.fileinfo,parser_e_header_dont_match_forward,_c^.objname^+'.'+_name);
                                       ...
                                       if _c^.is_class and
                                         ...
                                         begin
                                            { warn only if it is the first time,
                                              we hide the method }
                                            if _c=hp^._class then
                                              MessagePos1(hp^.fileinfo,parser_w_should_use_override,_c^.objname^+'.'+_name);
                                            { was newentry; (FK) }
                                            newdefentry;
                                            exit;
                                         end;
                                       ...
                                       if not(is_equal(procdefcoll^.data^.rettype.def,hp^.rettype.def)) and
                                         ...
                                         MessagePos1(hp^.fileinfo,
                                          parser_e_overloaded_methodes_not_same_ret,_c^.objname^+'.'+_name);
...
{
  $Log: hcgdata.pas,v $
  Revision 1.1.2.5  2002/11/07 16:50:59  pierre
   * several memory leaks removed

  Revision 1.1.2.4  2002/10/18 21:55:38  carl
    * alignment of rtti information (m68k only)
    * bytes are also aligned when target = 68000
    + atari target

  Revision 1.1.2.3  2001/04/21 00:28:09  carl
  * m68k updates

  Revision 1.1.2.2  2001/04/10 20:55:39  peter
    * better filepositions for warnings

  Revision 1.1.2.1  2001/01/08 15:10:37  pierre
   * use \$goto on
